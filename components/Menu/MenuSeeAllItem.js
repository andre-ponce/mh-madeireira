        ...
        <>
          ...
          <ul className="item__subcategorias collapse navbar-collapse nvl-1" id="departamentos">
            ...
            {linha.subMenus.map(departamento => <li key={departamento.id} className={`menu__item menu__item-subcategoria ${isEmpty(departamento.subMenus) ? '' : 'menu__item--has-subcategoria'}`}>
              <Link href={linkTo.category(departamento)} passHref><a>{departamento.nome}</a></Link>
                  ...
                  <ul className="subcategoria__group collapse navbar-collapse nvl-2" id="subdepartamentos">
                    {
                      departamento.subMenus.map(subDepartamento =>
                        <li key={subDepartamento.id} className="menu__item">
                          <Link href={linkTo.category(subDepartamento)} passHref><a>{subDepartamento.nome}</a></Link>
                        </li>
                      )
                    }
                  </ul>
